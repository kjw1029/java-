...
class LoginManager {
	...
	public String setUserWorkInInfo(String userName) {
		...
		for (User user : users) {
			if(userName.equals(user.getUsername())) {
				if(!"".equals(user.getInWorkTime()) || !user.getInWorkTime().isEmpty()) {
					timeInfo = user.getUsername() + "는/은 이미 출근 처리된 사용자입니다.";
					return timeInfo;
				}
				user.setInWorkTime(timeInfo);
				timeInfo = user.getUsername() + "는/은 현재 시각 " + user.getInWorkTime() + "에 정상 출근 처리되었습니다.";
			}
		}
		return timeInfo;
	}
	...
	public String setUserWorkOutInfo(String userName) {
		...
		for (User user : users) {
			if(userName.equals(user.getUsername())) {
				if(!"".equals(user.getOutWorkTime()) || !user.getOutWorkTime().isEmpty()) {
					timeInfo = user.getUsername() + "는/은 이미 퇴근 처리된 사용자입니다.";
					return timeInfo;
				}
				user.setOutWorkTime(timeInfo);
				timeInfo = user.getUsername() + "는/은 현재 시각 " + user.getOutWorkTime() + "에 정상 퇴근 처리되었습니다.";
			}
		}
		return timeInfo;
	}
	...
	public String getUserWorkInInfo(String userName) {
		...
		for (User user : users) {
			if(userName.equals(user.getUsername())) {
				if("".equals(user.getInWorkTime()) || user.getInWorkTime().isEmpty()) {
					userInfo = user.getUsername() + "는/은 출근 기록이 존재하지 않습니다.";
					return userInfo;
				}
				userInfo = user.getUsername() + "는/은" + user.getInWorkTime() + "시각에 출근했습니다.";
			}
		}
		return userInfo;
	}
	...
	public String getUserWorkOutInfo(String userName) {
		...
		for (User user : users) {
			if(userName.equals(user.getUsername())) {
				if("".equals(user.getOutWorkTime()) || user.getOutWorkTime().isEmpty()) {
					userInfo = user.getUsername() + "는/은 퇴근 기록이 존재하지 않습니다.";
					return userInfo;
				}
				userInfo = user.getUsername() + "는/은" + user.getOutWorkTime() + "시각에 퇴근했습니다.";
			}
		}
		return userInfo;
	}
