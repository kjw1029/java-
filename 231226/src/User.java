		...
		for(int i = 0; i < users.length;i++) {
			if(userName.equals(users[i].getUsername())) {
				if(!"".equals(users[i].getInWorkTime()) || !users[i].getInWorkTime().isEmpty()) {
					timeInfo = users[i].getUsername() + "는/은 이미 출근 처리된 사용자입니다.";
					return timeInfo;
				}
				users[i].setInWorkTime(timeInfo);
				timeInfo = users[i].getUsername() + "는/은 현재 시각 " + users[i].getInWorkTime() + "에 정상 출근 처리되었습니다.";
			} 
		}
		return timeInfo;
	}
	...
	public String setUserWorkOutInfo(String userName) {
		...
		for(int i = 0; i < users.length;i++) {
			if(userName.equals(users[i].getUsername())) {
				...
				timeInfo = users[i].getUsername() + "는/은 현재 시각 " + users[i].getOutWorkTime() + "에 정상 퇴근 처리되었습니다.";
			} 
		}
		return timeInfo;
	}
