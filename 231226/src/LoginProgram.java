				...
				try {
					...
					if (loggedInUser.isAdmin()) {
						...
						//	AttendanceSystem.AdminWork();
							workInAndOutSet(loggedInUser.isAdmin());
						} else if (Asellect == 3) {
							//매출관리
					...
					// 일반 계정일 경우 
						...
						if (Dsellect == 1) {
							...
							workInAndOutSetDesk(); // 데스크 청소용 출퇴근
...
//			scanner.close();
	 ...
	 */
	public static void workInAndOutSet(boolean isAdmin) { 
		...
		while (go) {
			...
			case "1":
				...
				String clockInEmployeeId = scanner.nextLine();
				resultMsg = loginManager.setUserWorkInInfo(clockInEmployeeId);
				System.out.println(resultMsg);
				break;
			case "2":
				System.out.print("사원 ID를 입력하세요: ");
				String clockOutEmployeeId = scanner.nextLine();
				resultMsg = loginManager.setUserWorkOutInfo(clockOutEmployeeId);
				System.out.println(resultMsg);
				break;
			case "3":
				go = false;
	...
	//데스크 청소 출퇴근 새로 생성
	public static void workInAndOutSetDesk() { 
		System.out.println("출퇴근 관리 시스템 정상 접속되었습니다.");
